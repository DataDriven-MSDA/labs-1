...
`bas.lm` function from this package later in the lab to implement Bayesian Model Averaging.

### The data

The data were gathered as a random sample of 935 respondents throughout the United States.  (This data set was released as part of the series *Instructional Stata Datasets for Econometrics* by  the Boston College Department of Economics
[@Wooldridge2000]) The result is a data frame where each row corresponds to a unique individual and columns represent variables about the individuals.
...
`urban`          | =1 if live in a Metropolitan Statistical Area
`sibs`           | number of siblings
`brthord`        | brthord
`meduc`          | mother's education (years)
`feduc`          | father's education (years)
`lwage`          | log weekly wage (dollars)
...
<li> The right-skewness of the data means that more respondents fall below the mean wage than above it. </li>
</ol>
```{r wage-dist}
...
<div id="exercise">
**Exercise:** Excluding `wage` and `lwage`, select two other variables that you think might be a good predictor of `wage`.  Visualize the relationships between `wage` and these two variables with appropriate plots.
</div>
```{r two-vars-eda}
...
A simple explanation for the variation in wages that we see in the data is that smarter people make more money.  The plot below visualizes a scatterplot between weekly wage and IQ score.
...
This plot is rather noisy.  While there does appear to be a linear relationship between IQ score and wage, IQ is a very crude predictor for wage.  Now, let's model this relationship with simple linear regression.
...
The Bayesian model specification assumes that the errors are normally distributed with a common variance.  A way to check this assumption is to look at the distribution of the residuals.  If the residuals are highly non-normal or skewed, the assumption of normally distributed errors is not warranted.
...
One way to accommodate the right-skewness in the data is to log-transform the dependent variable.  Note that this is only possible if the dependent variable only takes on positive values.  Let's try to fit a linear model with log-wage as the dependent variable.  Question 4 will be based on this log-model.
...
<div id="exercise">
**Exercise:** Examine the residuals of this model.  Is the assumption of normally distributed errors valid?
</div>
```{r log-resid}
# type your code for the Exercise here, and Knit
...
Recall that the posterior distribution of $\alpha$ and $\beta$ given $\sigma^2$ is normal, but marginally follows a $t$ distribution with $n-p-1$ degrees of freedom.  In this case, $p=1$, since IQ is the only predictor of log-wage included in this model.  Since $n$ is so large, we can use a normal-approximation to make inferences on $\alpha$ and $\beta$. 
...
This command tells R to include all covariates in the model with the exception of wage.  However, running this full model has a cost: we remove observations from our data.  This is because some measurements for birth order, mother's education, and father's education are missing.  By default, R's `lm` function does a complete-case analysis, and so it removes all observations with NA values in one or more fields.  This increases the posterior variance of our parameters, as there are fewer observations.

In order for our inferences to be valid, we assume that the data is missing at random.  For example, if all first-born children did not report their birth order, the data would not be missing at random.  We make this assumption and use the 663 complete observations (as opposed to the original 935) to fit the model.  Bayesian methods exist to use observations with missing data, though they are beyond the scope of this course.  
...
As you can see from a quick summary of the full linear model, many coefficients of independent variables are not statistically significant.  In previous labs within this specialization, you selected variables based on p-values and Adjusted $R^2.$  This module introduced the Bayesian Information Criterion (BIC), which is a metric that can be used for model selection.  BIC is based on model fit, while simultaneously penalizing the number of parameters in proportion to the sample size.  We can calculate the BIC of the full linear model using the command below:
...
```{r bas-wage}
bma_lwage = bas.lm(lwage ~ . -wage, data = na.omit(wage),
...
```{r vis-BMA}
par(mfrow = c(1,2))
plot.coef.bma(coefficients(bma_lwage), subset = c(3,13))
```


For questions 7-8, to give you practice with Bayesian model averaging, we'll use a reduced data set, which excludes number of siblings, birth order, and parental education.
...
Suppose you observe four numerical observations of $y$, which are 2, 2, 0 and 0 respectively.  Assuming that $y \sim N(\mu, \sigma^2)$, dnder the reference prior $p(\mu,\sigma^2) \propto 1/\sigma^2$, our posterior becomes

\begin{align*}
mu|\sigma^2 &\sim N(1, \sigma^2/4)
1/sigma^2 &\sim G(\alpha = 2,\beta = 2)
\end{align*}
